...
@pytest.fixture
def empty_project_folder(tmp_path: Path, run_cli: RunCli) -> Path:
...
def uv_project_folder_without_dep(
    tmp_path: Path, run_cli: RunCli, isolate_virtual_env: None
) -> Path:
...
@pytest.fixture
def complex_vars_project(tmp_path: Path) -> Path:
    bakefile_path = tmp_path / DEFAULT_FILE_NAME
...
@pytest.fixture
def ctx_test_project(tmp_path: Path) -> Path:
    bakefile_path = tmp_path / DEFAULT_FILE_NAME
